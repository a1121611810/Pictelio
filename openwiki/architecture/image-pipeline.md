...
2. **Local IntersectionObserver** — Always-active `createEverVisible` observer (removed `skipObserver` in v3.21.3). Elements entering the viewport independently trigger `loadImage()` regardless of `visiblePage`

The `shouldLoad()` memo combines both signals via OR — either condition being true triggers prefetch. This ensures the IntersectionObserver serves as an independent safety net when the parent observer locks to a stale page (e.g. the fix in `c05277f` where the parent now selects the highest intersecting `pageIndex` instead of the highest `intersectionRatio`).
...
**Preload window** (`PRELOAD_WINDOW = 3`): The expanded window from `+1` (v3.21.0) to `+3` ensures up to 4 concurrent prefetches at scroll start (pages 0–3). `PixivApiPlugin` configures OkHttp with a custom `Dispatcher` (cached thread pool), `callTimeout` set to connect+read timeout sum, `maxRequestsPerHost = 10`, and `maxRequests = 20` (v3.21.3+), so the 4 concurrent prefetches stay well within the host connection limit. See [ADR-0039](/docs/adr/ADR-0039-detail-image-cache-ready.md) for the full design rationale.
