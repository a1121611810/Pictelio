...
| `/recommended` or `/following` | `TabFeedPage.tsx` (loading-triggered) | TanStack Query `loading()` becomes `true` → `requestAnimationFrame` → splash dismiss (skeleton frame guaranteed visible before splash exit). 500ms fallback timeout. |
