...
- **Token storage:** [ADR-0050](/docs/adr/ADR-0050-lynx-login-persistence.md) — **web-core** uses IndexedDB for `refresh_token` persistence (Worker environment lacks `localStorage`; IndexedDB XSS risk accepted as MVP tradeoff). The storage backend is a **generic IndexedDB KV layer** ([`idbKV.ts`](/packages/app-lynx/src/utils/idbKV.ts), DB `pictelio_lynx` v2) with `idbSet`/`idbGet`/`idbRemove`; [`tokenStorage.ts`](/packages/app-lynx/src/utils/tokenStorage.ts) is a thin wrapper (`saveRefreshToken`/`loadRefreshToken`/`clearRefreshToken`) over this KV layer. **Native LynxView** (#41) will use a Lynx Native Module aligned with the main project's `@aparajita/capacitor-secure-storage` Keystore encryption for cross-client login sharing.
...
- **Image display:** Lynx's `<image>` component does not support `widthFix` mode (silently falls back to `fill`, causing zero-height or stretched images). Both `Recommended.vue` and `IllustDetail.vue` use the **[`SkeletonImage`](/packages/app-lynx/src/components/SkeletonImage.vue) component** — a wrapper that applies `aspectFill` mode with an explicit `aspect-ratio` container (`1/1` for square thumbnails, dynamic `${width} / ${height}` from the API for detail images) plus an optional `minH` vw fallback to prevent height collapse (ADR-0045). This avoids the `widthFix` → `fill` → 0-height pitfall documented in [glossary-web-core-pitfalls](/docs/adr/glossary-web-core-pitfalls.md).
