...
| `/recommended` or `/following` | `TabFeedPage.tsx` (`onMount`) | Component mounts → `markContentReady()` after 400ms delay. Simpler than the prior loading-triggered strategy because the native exit animation was removed (see commit `fa2015c`). |
...
>
> **v3.21.8+ (working tree, commit `fa2015c` + uncommitted 400ms delay):** The `onMount` handler adds `setTimeout(() => markContentReady(), 400)` to wait for the skeleton screen's first paint before dismissing the native splash. The immediate `markContentReady()` in v3.21.7 could produce a flash when the skeleton DOM had not yet rendered to screen. VirtualFeed skeleton condition was also hardened to `(loading || !loadAttempted)` so the skeleton is visible even during the brief pre-fetch window.
