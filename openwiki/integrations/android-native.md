...
- **Edge-to-edge base (ADR-0168):** `onCreate` calls `EdgeToEdge.enable(this)` (after `installSplashScreen`/`super.onCreate`, before `setContentView`) — one code path for API 28→35+ that makes the status-bar region paint with the app surface color and eliminates the prior two-form split (≤14 independent black/grey bars vs 15+ forced edge-to-edge with zero insets compensation). Status-bar icon color is then pinned via `isAppearanceLightStatusBars = true` (app-lynx has no dark UI). The three-key nav bar's 80% scrim is left at the compat default.
