...
## app-lynx Novel Intro Page & Three-Segment Navigation (ADR-0167)

Lynx novel navigation was asymmetric with illusts: illusts use a two-segment "list → detail" flow, but all six novel entry points (recommended carousel / novel list / bookmarks / user home / followed-latest chapter / search sheet) jumped straight to the body page, whose header carried no cover, tags, or intro — no pre-read decision surface (the `/v2/novel/detail` endpoint already returned `caption`/`tags`/`total_view`/`series`; they were just never rendered). ADR-0167 adds a **three-segment UI flow** on lynx: every novel tap now routes to [`NovelIntro.vue`](/packages/app-lynx/src/pages/NovelIntro.vue) at `/novel/:id/intro` (a `requiresAuth` route in [`router.ts`](/packages/app-lynx/src/router.ts)) first, while `/novel/:id` stays directly reachable — the three-segment flow is a **UI constraint, not a route constraint** (no route-level redirect; the benchNav test channel and future deep links depend on direct reachability). **Webview is out of scope** and keeps its two-segment flow.

The intro page reuses the recommended-carousel "D" visual family: a full-screen cover `aspectFill` + bottom gradient scrim (`--md-scrim-overlay` token) carrying cover (`CoverImage` three-state), AI badge, series row, title, author row (→ user home), tag capsules (reused `AdaptiveTagRow`; chip tap → global search sheet), a two-line intro (tap opens a `modalStack` bottom panel for full text), stats, comment entry, and a fixed bottom 「开始阅读」 CTA + bookmark. Optional fields degrade explicitly (section hidden / 「暂无简介」) — no silent fake data. Restricted/AI masking is same-source as the body page (`settings.isRestricted` / `isAiRestricted`, R-18 first then AI mask): cover/title/author stay visible, the intro area is masked, and the CTA + intro-expand entry grey out via handler guards (`if (masked.value) return`) rather than CSS alone. 「开始阅读」 always starts from the beginning (resume/progress recovery is out of scope), and the body header was simplified to title + author + comment/export entries (word count / bookmark / series row moved to the intro page; series data is still loaded via the detail endpoint because the watchlist prompt depends on it).

The intro page is also the **lynx client's first novel bookmark entry**: [`useBookmarkMutation`](/packages/app-lynx/src/composables/useBookmarkMutation.ts) gained an optional `targetKind` (`'illust' | 'novel'`, default `'illust'` — zero blast radius) that dispatches to the webview-aligned novel endpoints (`POST /v2/novel/bookmark/add`, `POST /v1/novel/bookmark/delete`); novel endpoints carry no tags payload, so `saveWith` invoked from a novel host explicitly `console.warn`s and saves per the restrict setting instead of silently dropping it. Defense lines: [`novelIntroEntryGuards.test.ts`](/packages/app-lynx/tests/novelIntroEntryGuards.test.ts) pins the intro navigation string per entry point plus a negative assertion against direct-body fallback and route coexistence/no-redirect; the intro page has template-level source guards (three-state / scrim / mask order / grey binding) and endpoint contract success/failure tests. See [ADR-0167](/docs/adr/ADR-0167-lynx-novel-intro-three-segment.md).

ADR-0183 makes the intro page **optional for high-frequency readers**: a device-level `novel_intro_first` setting (default `true` = intro-first, so upgrades are zero-perception) now lets any of the six entry points skip `/novel/:id/intro` and jump straight to the body. Navigation is funneled through a single [`openNovel(id)`](/packages/app-lynx/src/utils/novelNavigation.ts) seam that reads the switch and emits the intro or body route — the six inline `navigate(\`/novel/${id}/intro\`)` call sites were replaced so `/intro` strings now only exist in that module. The route layer is unchanged (no redirects; intro page and body page stay directly reachable per the ADR-0167 deep-link constraint), and the `novelIntroEntryGuards.test.ts` negative assertion was rewritten to pin the seam call instead of forbidding direct-body navigation. The toggle is a device-level setting (not account-scoped) surfaced as a `Me.vue` content-group `M3Switch` row (i18n `me.content.novelIntroFirst`); webview stays out of scope (it has no intro page). See [ADR-0183](/docs/adr/ADR-0183-lynx-novel-intro-toggle.md).
