# FT-5 交互回归 · 20260905-144421

| 场景 | 指标 | 值 | 判定线 | pass | 判定线出处 |
|---|---|---|---|---|---|
| coldstart | splash_first_ms | 524 | — | 记录 | 体检报告场景 1（splash 首帧=首个显著变化帧） |
| coldstart | max_stall_ms | 1461 | <1000 | **FAIL** | docs/research/ri-365-coldstart-report.md（FT-2 验收线：最长完全静止段 <1s） |
| coldstart | waittime_ms | 584 | — | 记录 | docs/research/ri-365-coldstart-report.md（暖基线 1782ms；网络/缓存波动，仅记录不设回归线） |
...
| back | variance_ms | 383 | <100 | **FAIL** | docs/research/real-interaction-audit.md 场景 3（响应 ≤400ms 且 3 rep 方差 <100ms） |
| back | note | rep1: 详情图 30s 未就绪（网络支配），按当前状态录制 | | | |
| tabs | follow_response_ms | 124 | ≤1100 | PASS | docs/research/real-interaction-audit.md 场景 4（响应 ≤1.1s） |
| tabs | follow_blank_window_max_ms | 0 | ≤500（空白窗=0） | PASS | docs/research/ri-366-shimmer-report.md（FT-3 验收线：空白窗=0；>500ms 无卡片段即 fail） |
| tabs | follow_content_ready_ms | 124 | — | 记录 | 体检报告场景 4 内容就绪（网络支配，仅记录） |
| tabs | bookmark_response_ms | 124 | ≤1100 | PASS | docs/research/real-interaction-audit.md 场景 4（响应 ≤1.1s） |
...
| tabs | bookmark_content_ready_ms | 124 | — | 记录 | 体检报告场景 4 内容就绪（网络支配，仅记录） |
| scroll | scroll_slow_delta_p90_ms | 17 | ≤34 | PASS | docs/research/real-interaction-audit.md 场景 5（VFR delta p90 ≤34ms） |
...
| viewer | open_ms | 44 | ≤200 | PASS | docs/research/real-interaction-audit.md 场景 6（开启占位 ≤200ms） |
| viewer | flip_max_gap_ms | 152 | ≤500 | PASS | docs/research/ri-367-viewer-report.md（FT-4 验收线：翻页释放→新页首帧 ≤500ms） |
