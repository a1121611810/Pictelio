...
2. **真实 LLM 端点已跑通一次，但行为不稳定**：`deepseek-chat` + 真实 API key 跑通了一次（截图 `app-lynx-translation-emulator-04-real-deepseek.png`：按钮「✓ 重译」、正文已替换为真实译文）。但**同一端点此前的运行返回 200 且零输出**（内容策略拒答），因此真实端点的译文质量与拒答率需按作品/模型分别抽样，不能据一次成功推断可用性。
...
- **gradle 只认 asset 变化时不重打包**：JS bundle 更新但 `assembleLynxDebug` 判 up-to-date → APK 落后于它应包含的 bundle。脚本现在**先删 APK** 再构建，保证打包必然发生。
- **新鲜度判据要看源集**：比「构建开始时刻」会在合法的 up-to-date 情况下误报；现比**打进 APK 的源集**（`main/java` + `lynx/java` + bundle），既放行 up-to-date 又能抓住「编译失败被吞」。
