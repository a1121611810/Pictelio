...
| `seq` | `number` | per-stream 单调序号，**在帧入缓冲时盖一次**（两路交付的是同一字符串）。<br>实现期修订（复审实测两次）：① 只给总线路径注入 → 轮询帧无 seq，去重单向失效；② 改为「两路都注入」仍不行 —— 交付时盖章会让同一帧经两路拿到**不同** seq（实测 0 vs 2），去重同样永不命中。唯一成立的做法是入缓冲时盖章。<br>接收侧据 `(streamId, seq)` 去重，避免同帧两路交付导致重复 append |
