...
> 范围：`packages/app-lynx` 的**全局搜索**功能——底部弹层命令面板（变体 A），从任意内容页唤起，搜索对象为**插画 + 小说**。配套 ADR：[ADR-0132](./ADR-0132-app-lynx-global-search.md)；形态蓝本:原型 `PrototypeSearch.vue`（throwaway 分支 `prototype/lynx-global-search`，不入 main）+ 研究 `docs/research/global-search-patterns.md`。**复用术语**（SearchScope、热门排序、next_url、mergeSearchResults、paginationError、内联重试模式、搜索目标）见 [glossary-search-pagination.md](./glossary-search-pagination.md)，本表不重复定义。
