...
### 已知未落地项（2026-09-20 #665 票面裁定）

| # | 要求 | 出处 | 最终状态 |
|---|---|---|---|
| 7 | spec §7.1 `translating` UI 反馈 = 「段落增量渲染 / 段落实时替换」 | issue #665（#640 step 2 取证） | ✅ **票面作废（幽灵验收项）**—— 实现是「进度百分比 + 内容终态整章切换」。架构硬约束 = lynx NativeModule 一次流只可靠投递一帧（ADR-0170），渐进粒度上限 = chunk 级（5 块）。spec §7.1 第 510 行已改写、§7.2 末尾「幽灵验收项作废」加第 3 条；按 #640 step 6 / #656 契约项 4 同模式处理。**UX 决策 = 终态切换 + 缓存命中 < 50ms**，避免半截译文混排 + 与 `displayBlocks` memo / 虚拟滚动 / 「先渲染后加载」硬约束冲突 |
