...
| 术语 | 定义 |
|------|------|
| **放射导航 FAB（radial nav FAB）** | 全局唯一的**右下角悬浮 FAB**，是 app-lynx 的导航中枢；点按展开成「双层环」放射菜单，替代原 `NavigationBar`（底部 4 tab）与各顶层页自己的刷新 FAB。在 4 个顶层 tab 页（推荐/插画/小说/我的）显示为放射菜单；**ADR-0131 起非 tab 内容页也显示**——形态为「搜索按钮（直达模式）」，非内容页（/login /update /error）隐藏，见 [glossary-app-lynx-global-search.md](./glossary-app-lynx-global-search.md)「FAB 搜索入口 / 直达模式」。_Avoid_: 底部导航栏、浮动按钮、FAB 菜单（与 feed 分页 FAB 菜单区分，见下） |
...
| **激活页（active page）** | 当前显示的路由对应 tab 页；只有它的动作会进入内环。从 `routeState.name` 与 4 tab 名匹配派生；非 tab 路由 → `active===null`（不展放射菜单；search/hidden 形态见「放射导航 FAB / 可见性门」，ADR-0131）。 |
...
| **可见性门（visibility gate）** | 显示门三态（ADR-0131 扩展自 ADR-0120 布尔门）：`view.mode === 'menu' ⟺ routeState.name ∈ 4 tab 名`；`'search'` ⟺ 内容页（除 /login、/update、/error 外全部路由）；`'hidden'` ⟺ 非内容页。`view.inner` 在 menu 模式含全局搜索项，search/hidden 模式无渲染（遮罩/环层随 `isOpen` 恒 false 不渲染）。 |
