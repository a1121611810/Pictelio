...
| 术语 | 定义 |
|------|------|
| **放射导航 FAB（radial nav FAB）** | 全局唯一的**右下角悬浮 FAB**，是 app-lynx 的导航中枢；点按展开成「双层环」放射菜单，替代原 `NavigationBar`（底部 4 tab）与各顶层页自己的刷新 FAB。在 4 个顶层 tab 页（推荐/插画/小说/我的）显示为放射菜单；**ADR-0132 起非 tab 内容页也显示**——形态为「搜索按钮（直达模式）」，非内容页（/login /update /error）隐藏，见 [glossary-app-lynx-global-search.md](./glossary-app-lynx-global-search.md)「FAB 搜索入口 / 直达模式」。_Avoid_: 底部导航栏、浮动按钮、FAB 菜单（与 feed 分页 FAB 菜单区分，见下） |
...
| **内环（inner ring）** | 双层环的内层：**全局搜索项（首位）** + **页面动作项**（刷新/回顶/翻页，由**激活页**注册的动作派生）。字段存在=该页有此内环项；**Me 页注册空动作 → 内环仅含搜索项**。 |
...
| **激活页（active page）** | 当前显示的路由对应 tab 页；只有它的动作会进入内环。从 `routeState.name` 与 4 tab 名匹配派生；非 tab 路由 → `active===null`（不展放射菜单；search/hidden 形态见「放射导航 FAB / 可见性门」，ADR-0132）。 |
| **读模型（view）** | 模块对外暴露的**单一只读响应式模型** `view: Readonly<Ref<FabView>>`：`{ mode, visible, active, isOpen, isBusy, outer, inner }`。组件只读它渲染，不再读散落的多份状态。`mode` = 显示门三态（menu/search/hidden，见「可见性门」），`visible` 为兼容别名（mode≠hidden）。 |
| **命令通道（dispatch）** | 模块对外暴露的**单一命令入口** `dispatch(cmd)`：`toggle / close / select{name} / refresh / back-to-top / extra{key} / search`。打开/关闭/选中/分发全部经它，由模块内状态机裁决互斥。_Avoid_: 暴露 `open()/close()/startRefresh()/endRefresh()` 等细粒度 setter。 |
| **busy 互斥（busy exclusion）** | 刷新或异步扩展项**进行中**时：`isOpen` 被强制关闭、`toggle/refresh/back-to-top/extra` 被忽略（no-op+warn）、中心 FAB 显示 spinner（`isBusy`）。`select`、`close`、`search` 始终允许（导航/收合/搜索入口打开与 busy 正交）。复用 `createFabMenuState` 作为**内部接缝**。 |
| **可见性门（visibility gate）** | 显示门三态（ADR-0132 扩展自 ADR-0120 布尔门）：`view.mode === 'menu' ⟺ routeState.name ∈ 4 tab 名`；`'search'` ⟺ 内容页（除 /login、/update、/error 外全部路由）；`'hidden'` ⟺ 非内容页。`view.inner` 在 menu 模式含全局搜索项，search/hidden 模式无渲染（遮罩/环层随 `isOpen` 恒 false 不渲染）。 |
...
| **展开层叠序（expanded stacking order）** | FAB 展开后，放射菜单项须**浮于半透明遮罩之上**：`遮罩(z-10) < 菜单项(z-20) < 主 FAB(z-30)`（同一 **z-40 外层**内）。ADR-0123 起外层为**钉在 (0,0) 的零尺寸盒**（只作定位锚点、不参与命中测试），遮罩与菜单项整层 `v-if="view.isOpen"` 条件渲染（关闭态渲染树无全屏元素 → 页面点击不被吞），主 FAB 常显于其内。ADR-0121 时期外层为**常显全屏容器**（关闭态吞页面点击，即 ADR-0123 修复的 bug）；更早（ADR-0121 之前）菜单项无 z-index（默认 0），被遮罩压住且点不到。 |
