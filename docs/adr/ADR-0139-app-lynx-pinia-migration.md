...
6. **globalFab 后置迁移已立项（2026-09-03 后续，详见 ADR-0140 + spec #343 + spike 报告 `docs/research/globalfab-pinia-spike-report.md`）**：本 ADR 决策 8 排除 `globalFab` 迁移的隐含理由（"形态不匹配 / 测试已主动绕开 / TDZ 接线"）经 spike 实证均不成立。`globalFab.ts` 形态（模块级单例 wiring）与 7 个已迁 store 完全同形；`createGlobalFab` primitive 与 wiring 严格解耦；Pinia `defineStore(name, factory)` 的 factory body 在模块加载期不执行，与原 `let _fab` 闭包懒求值同语义。**全量迁移**（实施 commit TBD）：① `stores/globalFab.ts` spike 清理（删除 `_factoryCallCount` / `console.log` / `__spikeFactoryCallCount` / 旧 `getGlobalFab`）② 4 页面（`Me.vue` / `IllustList.vue` / `NovelList.vue` / `Recommended.vue`）改 `useGlobalFabStore` ③ `GlobalFab.vue` 切 `useGlobalFabStore` + `storeToRefs` ④ 新增 `stores/globalFab.test.ts`（10 例覆盖 view 派生 / usePage / dispatch / 跨 store 闭包）。**实施发现**：spike 期间为防御性冗余留下的模块级 `_fabCache` 缓存**破坏测试隔离**（第一次 `useGlobalFabStore()` 后 `_fabCache` 捕获旧 `holder.routeState`，后续 `setActivePinia` 改了 ref 但旧 `createGlobalFab` 实例还在用旧 ref → computed 不响应）——**全量迁移时删除 `_fabCache`**，依赖 Pinia store id 'globalFab' 保证单例即可（active pinia 范围内 factory body 仅执行一次）。验证：780/780 vitest pass、lynx 759KB / web 734.7KB（+0.4% / +0.6% 在合理范围）。
7. **globalFab 后置迁移按单 commit 提交（2026-09-03 实施，commit `9188af8f`）**：与 D9 让步先例一致——globalFab 迁移涉及 stores/globalFab.ts + 4 页面 + GlobalFab.vue + 新 test 文件 7 处源码变更 + 4 处文档变更，原子行为单元（spike → 全量迁移的连贯决策）拆 commit 会割裂；按 ADR-0140 修订注记 2 处方执行。code-review 双轴（Standards + Spec，含调用点完备性 + 期望值溯源）已通过：0 阻塞 finding，3 重要 + 9 建议 finding 全部就地修复并 amend commit（详见 commit message）。
