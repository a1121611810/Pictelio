...
8. **原生选区清除（已实证有效）**：用 `setTextSelection` 退化参数（全负坐标 + 不显示手柄）清除引擎侧选区。**设备实验（无点击干扰的延时执行）**：调用后手柄像素 382→0、高亮回到背景基线（133）；对照组不调用则原样保留（380/273）。备选方案均无增益：零长 range 清不干净（残留 232）、极小区间与全节点重复调用与现状等价、整表重建虽最彻底但会把滚动位置顶回顶部。清选调用失败只 warn 一次、不阻断收起。
   **测量坑（记录以免重蹈）**：列表的竖向滚动指示条是 #0060FF 的细线，会被「蓝色手柄」像素过滤器计入 —— 早前「滚动收起后高亮残留」的观察即此伪影；判定选区是否清除必须看**手柄形状**（成对圆点）或直接肉眼核对截图。
