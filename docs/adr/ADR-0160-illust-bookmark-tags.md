...
> **勘误（2026-09-21，票 #707 / spec `docs/specs/app-lynx-recommended-novel-bookmark.md`）**：上文裁剪理由中「小说收藏入口在列表卡片」**与代码事实不符**——两侧客户端的小说**列表卡都没有收藏入口**（app-lynx `NovelList` 零处 `BookmarkButton`；webview `NovelRowCard` 仅只读展示收藏数）。当时真实存在的小说收藏入口只有**小说介绍页底部 ♥**；本票又补上**推荐轮播小说滑页 ♥**（与插画滑页同槽位同形）。裁剪结论（小说收藏**加标签**不在本期）不变，变更的只是「入口在哪」这一事实前提。
