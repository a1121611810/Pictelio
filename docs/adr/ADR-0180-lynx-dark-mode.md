...
- **机器防线（四类契约测试，防回潮）**：
  ...
  - **跨语言色值**（同文件）：`values-night` 暗面 ≡ `.theme-sky.dark --md-surface`；暗 plate ≠ 暗面（离底有差 → **兜底轨**前景圆盘可见；主轨生效性见走查 T3-3c）；
