...
| **非命中层（inert layer）** | 从命中测试中移除、不参与触摸路由的层。实现手段只有两种：**`v-if` 不渲染**（关闭态最常用）或**零尺寸盒**（`absolute` + 无 inset，子元素仍可渲染）。`pointer-events: none` 在原生 LynxView **不是**合法手段。 |
...
| **展开层叠序（expanded stacking order）** | 放射 FAB 展开后的层叠：**遮罩(z-10) < 菜单项(z-20) < 主 FAB(z-50)**。遮罩与菜单项同处 `z-40` 展开层；主 FAB 为**独立常显层**（`z-50`，展开层之外的兄弟节点，关闭态仍存在）。修复前主 FAB 在容器内 `z-30`（ADR-0121）；本次因展开层改 `v-if` 条件渲染，主 FAB 移出为独立层并提至 `z-50`，相对次序不变。 |
