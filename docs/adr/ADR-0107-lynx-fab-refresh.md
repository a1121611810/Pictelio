...
3. **原生面整体回滚至 ADR-0106 前基线**：移除 `xelement-refresh:4.0.1`、`refresh-layout-kernel:3.0.0-alpha`（传递）、`androidx.viewpager2:viewpager2:1.1.0`（补漏）三个新增依赖；删除 verifyPinnedAars SHA-256 校验任务与 proguard keep 规则；删除未提交的 `PictelioAppModule.finishRefresh` @LynxMethod。JS 可调原生方法与供应链依赖面零增量。历史处理：rebase drop T1 提交（918f731，未推送），失败知识由本 ADR 承载而非 git 历史。
4. **移除 refreshEpoch workaround**：patch 错位仅在 `<refresh>` 包裹下观测到；T3 前推荐页 FAB 刷新（裸 list 数据替换）在原生长期正常。模拟器验证裸 list 无错位即确认；若复现（证明与 `<refresh>` 无关），仅该页恢复 `:key` 重建并上报框架 bug，不推广。
...
- 正面：
  - 原生依赖面与 JS↔native 方法面**回退到 ADR-0106 前基线**（-3 依赖、-1 @LynxMethod、-66 行校验任务、-2 proguard keep），供应链与攻击面零增量。
  - 刷新知识收口点从 4 处（组件 + 原生桥 + build.gradle + 9 页状态机）减到 **1 处**（RefreshableList.vue）；页面侧刷新代码净删约 6 行/页。
  - 无 `<refresh>` 包装层 → 无 SmartRefreshLayout 原生视图常驻、无 refreshEpoch 整树重建（刷新峰值无双 FiberElement 树并存）。
...
- 待验证项（T4' 模拟器验收闭环）：
  - 裸 view 容器（`relative flex-1 min-h-0`）内 list `h-full` 的原生解析；不解析则备选 `flex-col` + list `flex-1 min-h-0`（T3 前已验证结构，改动 2 行）。
  - 裸 list 数据替换无 patch 错位（决策 4）。
