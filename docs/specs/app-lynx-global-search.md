...
> 状态：ready-for-agent · 关联：ADR-0131 / glossary-app-lynx-global-search / issue #60（差距 #1）· 蓝本：webview `Search.tsx` + `api/search.ts` + `searchStore.ts` · 定案：用户 2026-09-01（变体 A + FAB 双形态入口 + 即输即搜）
