...
| D4 | 状态栏图标深浅固定 | `isAppearanceLightStatusBars = true` 恒定（enableEdgeToEdge 之后覆盖调用）：app-lynx 无暗色 UI（全仓零 `prefers-color-scheme`），浅色底配深图标；lynx 暗色模式落地时再联动。**已修订（2026-09-21）**：lynx 暗色模式落地——改为随 `resolvedDark` 动态（亮→true / 暗→false），全屏模式跳过；见 [ADR-0180](../adr/ADR-0180-lynx-dark-mode.md) D6 | #592 F4.1：e2e 下唯一正规途径；避免系统暗色时 enableEdgeToEdge 联动出白图标 |
