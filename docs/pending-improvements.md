...
2. **错误消息不够具体**
   ...
   - 用户无法从提示中得知具体应检查本地代理（默认 `127.0.0.1:7897`）是否运行。
...
- 返回 `{ type: ApiErrorType.PROXY, message: "本地代理连接失败（127.0.0.1:7897），请检查代理软件是否运行" }`
...
| 类型 | 指引文字 | 按钮 |
|---|---|---|
| `PROXY` | "本地代理连接失败（127.0.0.1:7897），请检查代理软件是否运行" | "检查代理设置" → 打开扫描页面 |
