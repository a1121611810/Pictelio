...
import { parseSnapshot, type RestorePlan } from "./backupCore";
...
function deps(overrides: Partial<BackupDeps> = {}, canned: {
...
describe("backupService — URL 与文件名（spec §3.4/§7）", () => {
  it("dirUrlOf 归一化斜杠；dir 为空回落服务器根", () => {
    expect(dirUrlOf({ url: "https://dav/", username: "", dir: "Pictelio/backup", excludedKeys: [] })).toBe(
      "https://dav/Pictelio/backup/",
    );
    expect(dirUrlOf({ url: "https://dav", username: "", dir: "", excludedKeys: [] })).toBe("https://dav/");
  });
...
describe("backupService — 选档筛选（spec §7 恢复入口）", () => {
  it("只保留备份前缀 + .json/.json.enc，剔除目录与无关文件，按时间倒序", () => {
    const files = selectBackupFiles([
      ...
      { href: "/dav/backup/pictelio-backup-20260901-120000.json", isCollection: false, contentLength: 10 },
      { href: "/dav/backup/pictelio-backup-20260903-120000.json.enc", isCollection: false, contentLength: 20 },
      { href: "/dav/backup/other-file.txt", isCollection: false, contentLength: 5 },
      { href: "/dav/backup/pictelio-backup-20260902-120000.json", isCollection: false, contentLength: 15 },
    ]);
...
describe("backupService — 备份流程（spec §5）", () => {
  it("明文路径：收集→快照→MKCOL→上传校验→旋转，快照字段正确", async () => {
    ...
    expect(calls.ensureDir).toEqual(["https://dav.example.com/remote.php/dav/files/alice/Pictelio/backup/"]);
    ...
    expect(new Set(Object.keys(snapshot.accountKeys))).toEqual(new Set(["show_r18_42", "show_r18_99"]));
...
describe("backupService — 恢复流程（spec §6）", () => {
  ...
  it("schema 过高 → SCHEMA_TOO_NEW 透传（spec §6 拒绝边界）", async () => {
    ...
    const err = await restoreFrom(d, { name: "pictelio-backup-x.json", encrypted: false }).catch((e) => e);
...
describe("backupService — 列档与连接测试（spec §7）", () => {
  it("listBackups 经 list + 筛选返回时间倒序", async () => {
    const { deps: d } = deps({}, {
      list: [
        { href: "/dav/backup/pictelio-backup-20260901-120000.json", isCollection: false, contentLength: 1 },
        { href: "/dav/backup/pictelio-backup-20260902-120000.json", isCollection: false, contentLength: 2 },
      ],
    });
...
describe("backupService — 自动备份判定（spec §7 T8）", () => {
  it("maybeAutoBackup：开关关 → 跳过；从未备份 → 执行；未到期 → 跳过；到期 → 执行", async () => {
    const { maybeAutoBackup } = await import("./backupService")
