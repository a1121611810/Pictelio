...
export function summarize(tasks: readonly DownloadTask[], ids: readonly string[]): SelectionSummary {
  const chosen = tasks.filter((t) => ids.includes(t.id))
  return {
    count: chosen.length,
...
export function allSelected(tasks: readonly DownloadTask[], selected: ReadonlySet<string>): boolean {
