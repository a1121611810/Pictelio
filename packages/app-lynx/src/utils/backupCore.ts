...
// 与 app utils/backupCore.ts 同源同语义（downloadManager 差分对齐约定）。
...
// ── UTF-8 编解码（纯 JS 单一事实源：utils/utf8.ts）──
// Lynx JS runtime 不提供 TextEncoder/TextDecoder（2026-09-11 备份链路实测、
// 2026-09-19 翻译链路复测）。实现已收敛到 utils/utf8.ts，此处仅转发导出，
// 保持既有 import 路径与测试不变。
...
/**
 * 字节 → 快照 v1（解析 + 校验）。
 * 拒绝边界（spec §6）：NOT_BACKUP / SCHEMA_TOO_NEW / CORRUPT。
 */
 ...
 * 规则（spec §6）：deviceKeys 全量；accountKeys 仅当前 uid；excludedKeys 一律不写；
 * 备份中不存在的键不出现在 apply。
 */
