...
<template>
  ...
  <view class="w-full h-full flex flex-col bg-surface">
    ...
    <view class="flex flex-row items-center justify-center h-[17.067vw] px-4 bg-surface">
      <text
        class="text-title-large font-medium text-on-surface"
    ...
    <scroll-view scroll-orientation="vertical" class="w-full flex-1">
      ...
      <GlassCard class="mt-3 mx-3 p-4">
        <view v-if="currentUser" class="flex flex-row items-center pb-4 border-b-[1px] border-b-outline-variant">
          ...
          <view class="ml-4 flex flex-col">
            <text class="text-headline-small font-bold text-on-surface">{{ currentUser.name }}</text>
            <text class="text-body-small text-on-surface-variant mt-1">@{{ currentUser.account }}</text>
          </view>
        </view>
        <view
          ...
          @tap="openBookmarks"
        >
          <text class="text-title-medium text-on-surface">我的收藏</text>
          <text class="text-title-medium text-on-surface-variant">›</text>
        </view>
      </GlassCard>
      ...
      <view v-if="supportsClientSwitch(availableKinds)" class="bg-surface-container-lowest mt-3 mx-3 p-4 rounded-[var(--md-shape-medium)]">
        <text
          class="text-title-small font-medium text-on-surface"
          :accessibility-element="A11Y_ELEMENT_ENABLED"
          :accessibility-label="ME_A11Y_LABELS.clientGroupTitle"
          >客户端</text
        >
        <text class="text-label-medium text-on-surface-variant mt-1 mb-3">选择渲染引擎后保存并重启生效</text>
        <view
        ...
        >
          <view class="flex flex-col">
            <text
              ...
              >WebView（现有）</text
            >
            <text class="text-label-medium text-on-surface-variant mt-0.5">SolidJS + Capacitor</text>
          </view>
          <!-- M3 radio button：选中 primary 实心 + on-primary 圆点，未选 outline 空心 -->
          <view
            class="w-[5.333vw] h-[5.333vw] rounded-full flex items-center justify-center border-2"
            :class="selectedClient === 'webview' ? 'border-primary' : 'border-outline'"
          >
            <view v-if="selectedClient === 'webview'" class="w-[2.667vw] h-[2.667vw] rounded-full bg-on-primary" />
          </view>
        </view>
        <view
          class="flex flex-row items-center justify-between py-3.5 border-b-[1px] border-b-surface-variant"
        ...
        >
          <view class="flex flex-col">
            <text
              ...
              >Lynx（当前）</text
            >
            <text class="text-label-medium text-on-surface-variant mt-0.5">vue-lynx 原生渲染</text>
          </view>
          <view
            class="w-[5.333vw] h-[5.333vw] rounded-full flex items-center justify-center border-2"
            :class="selectedClient === 'lynx' ? 'border-primary' : 'border-outline'"
          >
            <view v-if="selectedClient === 'lynx'" class="w-[2.667vw] h-[2.667vw] rounded-full bg-on-primary" />
          </view>
        </view>
      ...
      <view class="bg-surface-container-lowest mt-3 mx-3 p-4 rounded-[var(--md-shape-medium)]">
        <text class="text-title-small font-medium text-on-surface">内容</text>
        <text class="text-label-medium text-on-surface-variant mt-1 mb-3">默认隐藏 R-18 / R-18G 内容</text>
        <view
          class="flex flex-row items-center justify-between py-3.5 border-b-[1px] border-b-surface-variant"
          :accessibility-element="A11Y_ELEMENT_ENABLED"
          ...
          @tap="toggleR18"
        >
          <text class="text-title-medium text-on-surface">显示 R-18 内容</text>
          ...
          >
            <view class="rounded-full shadow-[var(--md-elevation-1)]" :class="showR18 ? 'w-[6.4vw] h-[6.4vw] bg-on-primary' : 'w-[4.267vw] h-[4.267vw] bg-outline'" />
          </view>
        </view>
        <view
          class="flex flex-row items-center justify-between py-3.5 border-b-[1px] border-b-surface-variant"
          ...
          @tap="toggleR18G"
        >
          <text class="text-title-medium text-on-surface">显示 R-18G 内容</text>
          <view
            class="w-[13.867vw] h-[8.533vw] rounded-full p-[0.8vw] flex flex-row transition-colors duration-[var(--durationNormal)] ease-[var(--motion-standard)]"
            :class="showR18G ? 'bg-primary justify-end' : 'bg-surface-container-highest justify-start'"
          >
            <view class="rounded-full shadow-[var(--md-elevation-1)]" :class="showR18G ? 'w-[6.4vw] h-[6.4vw] bg-on-primary' : 'w-[4.267vw] h-[4.267vw] bg-outline'" />
          </view>
        </view>
      </view>
      ...
      <!-- T6：动图播放组 -->
      <view class="bg-surface-container-lowest mt-3 mx-3 p-4 rounded-[var(--md-shape-medium)]">
        <text class="text-title-small font-medium text-on-surface">动图播放</text>
        <text class="text-label-medium text-on-surface-variant mt-1 mb-3">Ugoira 动图取帧方式</text>
        ...
        <view class="flex flex-row gap-0 rounded-[var(--md-shape-full)] border border-outline-variant overflow-hidden">
          <view
            ...
            :class="ugoiraMode === 'fflate' ? 'bg-secondary-container' : 'bg-transparent'"
            :accessibility-element="A11Y_ELEMENT_ENABLED"
            :accessibility-label="ME_A11Y_LABELS.ugoiraFflate"
            @tap="pickUgoiraMode('fflate')"
          >
            <text class="text-label-large" :class="ugoiraMode === 'fflate' ? 'text-on-secondary-container' : 'text-on-surface-variant'">fflate（默认）</text>
          </view>
          <view
            class="flex-1 py-2.5 flex items-center justify-center"
            :class="ugoiraMode === 'range' ? 'bg-secondary-container' : 'bg-transparent'"
            :accessibility-element="A11Y_ELEMENT_ENABLED"
            :accessibility-label="ME_A11Y_LABELS.ugoiraRange"
            @tap="pickUgoiraMode('range')"
          >
            <text class="text-label-large" :class="ugoiraMode === 'range' ? 'text-on-secondary-container' : 'text-on-surface-variant'">Range 流式</text>
          </view>
        </view>
        <text class="text-label-medium text-on-surface-variant mt-2 leading-snug">
      ...
      <view class="bg-surface-container-lowest mt-3 mx-3 p-4 rounded-[var(--md-shape-medium)]">
        <text class="text-title-small font-medium text-on-surface">详情画质</text>
        <text class="text-label-medium text-on-surface-variant mt-1 mb-3">列表缩略图 / 详情大图清晰度</text>
        <!-- M3 segmented button（三档）：容器 outline-variant 边框 + 全圆角，选中段 secondary-container -->
        <view class="flex flex-row gap-0 rounded-[var(--md-shape-full)] border border-outline-variant overflow-hidden">
          <view
            class="flex-1 py-2.5 flex items-center justify-center"
            :class="detailQuality === 'medium' ? 'bg-secondary-container' : 'bg-transparent'"
            :accessibility-element="A11Y_ELEMENT_ENABLED"
            :accessibility-label="ME_A11Y_LABELS.detailQualityMedium"
            @tap="pickDetailQuality('medium')"
          >
            <text class="text-label-large" :class="detailQuality === 'medium' ? 'text-on-secondary-container' : 'text-on-surface-variant'">标准</text>
          </view>
          <view
            class="flex-1 py-2.5 flex items-center justify-center"
            :class="detailQuality === 'large' ? 'bg-secondary-container' : 'bg-transparent'"
            :accessibility-element="A11Y_ELEMENT_ENABLED"
            :accessibility-label="ME_A11Y_LABELS.detailQualityLarge"
            @tap="pickDetailQuality('large')"
          >
            <text class="text-label-large" :class="detailQuality === 'large' ? 'text-on-secondary-container' : 'text-on-surface-variant'">高清</text>
          </view>
          <view
            class="flex-1 py-2.5 flex items-center justify-center"
            :class="detailQuality === 'original' ? 'bg-secondary-container' : 'bg-transparent'"
            :accessibility-element="A11Y_ELEMENT_ENABLED"
            :accessibility-label="ME_A11Y_LABELS.detailQualityOriginal"
            @tap="pickDetailQuality('original')"
          >
            <text class="text-label-large" :class="detailQuality === 'original' ? 'text-on-secondary-container' : 'text-on-surface-variant'">原图</text>
          </view>
        </view>
      </view>
    ...
    <view v-if="ugoiraConfirm" class="fixed inset-0 bg-scrim z-50 flex items-center justify-center">
      <view class="w-[74.667vw] max-w-[74.667vw] bg-surface-container-lowest rounded-[var(--md-shape-extra-large)] px-6 pt-5 pb-3 shadow-[var(--md-elevation-3)]">
        <text class="text-headline-small font-medium text-on-surface">切换到 Range 流式？</text>
        <text class="text-body-medium text-on-surface-variant mt-2 leading-snug">
          Range 流式按需取帧、内存更低；原生端依赖 Range 支持，个别网络环境可能更慢。
        </text>
