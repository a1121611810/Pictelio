...
// 长按静音 = ADR-0187 D5 / #732：长按 500ms 上抛 tag-long-press，页面层 muteTag + 轻提示）。
...
// 长按 → emit('tag-long-press', 原始 name)，由页面层决定行为，本组件可独立渲染/测试。
// +N 折叠芯片**不可点**（是计数不是标签，无搜索语义）。
...
const emit = defineEmits<{
  ...
  /** 长按标签 chip（携带原始 tag.name，页面层静音，ADR-0187 D5）；+N 折叠芯片不触发 */
...
<template>
  <view v-if="result.chips.length > 0" class="flex flex-row gap-1">
    <!-- key 用原始 name（text 可能因翻译名折叠出相同文本，文本 key 会冲突）。
         点击 → emit('tag-tap', 原始 name)（ADR-0133 决策 1/6）；长按 → tag-long-press（#732）。
    ...
    <TagPressChip
      ...
      :key="chip.name"
      :text="chip.text"
      chip-class="flex items-center justify-center bg-secondary-container rounded-[var(--md-shape-small)] px-2 py-0.5"
