...
// search 模式堆叠偏移（ADR-0131 决策 2「双 FAB 竖排堆叠」——几何推导，P1-1 修复）：
...
const fabCySearch = screenHeightVw() - FAB_BOTTOM_MARGIN_SEARCH_VW - FAB_SIZE_VW / 2
 ...
 *  search 模式（ADR-0131）较 menu 模式上移一档：feed 分页 FAB 同角竖排堆叠不遮挡。 */
const fabStyle = computed<Record<string, string>>(() => ({
  left: `${fabCx}vw`,
  top: `${view.value.mode === 'search' ? fabCySearch : fabCy}vw`,
...
/** 主 FAB tap：search 模式 = 直达搜索（不发 toggle、不展开放射菜单，ADR-0131 决策 2）。 */
...
<template>
  ...
  <view v-if="view.visible" class="absolute z-40" style="top: 0; left: 0">
    ...
    <!-- 主 FAB：menu 模式展开成 close、收起为当前 tab 图标（busy 时转圈/禁用）；
         search 模式（ADR-0131 直达模式）FAB 本体即搜索按钮（🔍），点按 dispatch('search')，
