...
// 形态对齐 api/illust.ts：client GET、next_url 透传（绝对 URL 由 client.rewriteUrl 剥域，ADR-0104）、
// 错误上抛（HTTP 非 2xx 由 client 归一为 ApiError，401 走 execWithAuthRetry 单飞刷新）。
// Java 零改动（PictelioApiModule.request 为 path 直拼、无白名单，ADR-0188 背景）。
