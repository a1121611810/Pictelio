 ...
 */
export function addHistory(word: string): void {
...
export function removeHistory(word: string): void {
  const trimmed = word.trim()
...
/** 测试专用：重置内存态（模块级单例，避免用例间串扰；不写盘） */
export function resetSearchHistoryForTest(): void {
  _history.value = []
}
