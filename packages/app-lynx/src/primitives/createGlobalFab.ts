...
/** FAB 显示门三态（ADR-0132 决策 2，扩展自 ADR-0120 的可见性门）：
...
/** 全局搜索项标注（ADR-0132）：内环「搜索」项与 search 模式主 FAB 共用的
...
/** 全局内置搜索内环项（ADR-0132 决策 2）：所有 tab 页内环固定首位，页面动作项顺延。 */
...
/** 非内容页路由名（router.ts routes[]，ADR-0132 决策 2：会话/系统页不提供搜索入口） */
...
export interface CreateGlobalFabDeps {
   ...
   *  FAB 与弹层互斥（issue #295 / ADR-0132）：任一弹层打开时 FAB 隐藏——
  ...
  async function dispatch(cmd: FabCommand): Promise<void> {
    switch (cmd.type) {
      ...
      case 'extra':
        ...
        return
      case 'search':
        // 全局搜索入口（ADR-0132 决策 2）：收起菜单 + 打开搜索弹层。
