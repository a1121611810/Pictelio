...
// 该 composable 是 IllustList / NovelList / Bookmarks / Following / UserHome 六个列表
// 组装点的共享过滤缝（useAiOnlyVisible 链式后置）；Recommended/SearchSheet 同谓词内联。
// 期望值 oracle = spec docs/specs/tag-mute.md 边界 1（空 tags/undefined 放行）+ ADR-0187 D2
// （trim 后精确相等）。真实 settingsStore（真实谓词）+ 登录态 mock。
...
describe('useTagMuteVisible（列表组装点静音过滤缝）', () => {
  ...
  it('链式组合 useAiOnlyVisible 形态：ComputedRef 输入可再过滤且响应集合变化', async () => {
    const items = ref<Item[]>([
      { id: 1, tags: [{ name: '風景' }] },
    ...
    settings.muteTag('R-18G')
    expect(visible.value.map((i) => i.id)).toEqual([1]) // 集合变化 → computed 重算
  })
})
