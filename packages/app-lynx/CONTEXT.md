...
系列小说详情页的返回守卫询问。触发条件全部命中才弹：小说是系列作品、未追更、本会话未「暂不」、页面停留 ≥10s，且（滚动进度 ≥70% **或**到达底部）。弹出时机 = **按返回键时**（返回守卫拦截），不是到底自动弹（已否决的语义）。「追更」加入追更列表后继续返回；「暂不」继续返回且本会话不再询问；返回键关弹窗 = 留在详情页且本会话不再询问。系列追更状态预取失败（状态未知）时保守不弹。三段式导航改造后询问**仍在正文页返回键触发**（地图 #575 / 票 #582：时机与条件不变，介绍页返回不触发；注意正文页头部精简后须保留系列判定数据）。
_Avoid_: 二次确认（口语别名，勿入代码与文档）、到底自动弹窗

### 小说导航（Novel navigation）【2026-09-17 新增，地图 #575】

**小说介绍页（novel intro page）**：
列表 → 介绍页 → 正文三段式导航的中间页（新路由 `/novel/:id/intro`）。承载封面（1:1，`CoverImage` 三态）/ 标题 / 作者行（→用户主页）/ 标签胶囊行（可点→全局搜索弹层，复用「标签自适应折叠」）/ 简介全文 / 统计行（字数·收藏·浏览·评论）/ 系列行（系列名+追更 chip）/ AI 徽章 / 评论入口 / 底部固定「开始阅读」主按钮 + 收藏。元信息数据全部来自 `/v2/novel/detail` 既有字段（`caption`/`tags`/`total_view`/`series`，零新增 API）。视觉方向（#575 原型 4 案对比定案，#583）：**D 轮播同族**——封面全屏铺满（aspectFill）+ 底部渐变 scrim 承载全部信息，页内不滚动，「开始阅读」+ 收藏嵌 scrim 底部；受限态 = 简介位遮罩 + CTA 置灰。scrim 仅容 2 行截断，**全文简介 = scrim 内展开**（#584：点简介弹 modalStack 底部面板读全文，受限态入口置灰）。
_Avoid_: 列表卡内展开简介（介绍语义收敛到独立页）、在介绍页放导出/分享（首批不做）、表单滚动式介绍页布局（方向已定全屏 scrim）

**三段式导航（list → intro → reader）**：
所有小说列表入口（推荐轮播 / 小说列表 / 收藏 / 用户主页 / 追更最新章节 / 搜索弹层）点击**一律先进介绍页**，正文页（`NovelDetail`）仅经介绍页「开始阅读」可达。**「只能」约束的是 UI 导航而非路由存在性**——`/novel/:id` 保留路由可达性（benchNav 测试通道、未来合法深链）。
_Avoid_: 列表直达正文的新增入口、把 `/novel/:id` 路由级重定向（测试通道会被打死）

**小说收藏入口（novel bookmark entry）【2026-09-21 新增，票 #707】**：
用户对小说执行收藏动作的入口。当前共两类：**小说介绍页底部 ♥**（主入口），以及**推荐轮播小说滑页 ♥**（与插画滑页同槽位同形、含收藏数；票 #707 / spec `docs/specs/app-lynx-recommended-novel-bookmark.md`）。动作均为「快速收藏」（按钮在本组件，收藏态与计数来自响应字段）。**列表形态的宿主均无入口**（小说列表 / 收藏页小说 tab / 用户主页小说 tab / 搜索弹层；webview 端同款）——这是已知不一致且已登记，不是待发现的缺陷。小说收藏**标签**（收藏面板）不在本期（ADR-0160 D7）。
_Avoid_: 把「小说可收藏」理解成列表卡可收藏（列表卡只读展示收藏数）；把推荐轮播的 ♥ 当成新导航入口（`@tap.stop`，点 ♥ 不进介绍页）
...
BookmarkButton 切换收藏的双向即时视觉反馈（M3 规范内形态）：收藏 = state-layer 环向外扩散 + 心形 spring 弹入并进入**已收藏配色**（chip 底 `tertiary` / 心形 `on-tertiary`）；取消收藏 = 环向内收拢（「收回」语义）+ 心形下沉回稳并回到**未收藏配色**（chip 底 `inverse-surface` / 心形 `inverse-on-surface`）。乐观触发（点按即播，失败静息回滚），change 事件在动画播完后才上抛（动画完成态）。详见 ADR-0112。
_Avoid_: 等 API 成功再播动画、粒子爆发、失败时播反向动画、用颜色名（「填红/褪灰」）描述状态（配色是可改的呈现细节，语义只有「已收藏/未收藏」）
...
**直连模式（Direct access）**【2026-09-06 新增】：
跨上下文共享概念——图片/API/OAuth 刷新三层的免梯网络通路（IP 直连 + 无 SNI TLS），实现在双引擎共享的 Java 传输层，lynx 客户端自动获得，JS 侧仅设置开关入口。术语详见 `packages/app/CONTEXT.md`「网络直连」。

**图床下载源（Image host download source）**【2026-09-06 新增】：
跨上下文共享概念——图床模式（单一/负载均衡/最快 IP）在缓存 miss 时决定实际取数渠道，决策收敛在双引擎共享的 Java 下载层，lynx 静态图与 ugoira zip 自动继承 webview 侧配置的图床（lynx 无设置 UI）。术语详见 `packages/app/CONTEXT.md`「图床」。
...
**滚动中采样（rolling-window sampling）**【2026-09-02 新增，ADR-0137】：
验证「滚动进行中出现、停止后消失」的瞬态 UI（滚动指示条/下拉刷新/滚动进度类）的**真机采样方法**——`adb input swipe`（≥1000ms 长滚动）与 `exec-out screencap` **并发**（滚动期间采样，shell `&` 并行时序），而非「滚动结束后截图」。根因：这类 UI 只在滚动中存活（指示条停止 500ms 后淡出），滚动后截图必然错过——agent 曾反复「看不见」而用户人眼可见（人眼连续观察 vs 单帧截图时序差）。**"滚动后截图"是已验证失败的验证姿势**。
_Avoid_: 滚动结束后截图验证滚动态 UI（必错过淡出窗口）、用 r<140 强阈值扫半透明指示条（须 r<200 + 右缘 x 1000-1080）
