...
列表页回顶入口：固定于列表容器右下角、刷新 FAB 上方的悬浮按钮（M3 small FAB 40dp）。滚动超过阈值（默认 800px）才显示（不常驻），点击平滑回顶（原生 scroll-to-index smooth，ADR-0109）。双端同构（原生 LynxView / web-core 同一实现与动画）。
_Avoid_: 常驻显示（遮挡内容）、JS 逐帧驱动回顶（原生 SmoothScroller 是正道）、首页/短列表显示（阈值天然过滤）
