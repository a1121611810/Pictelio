...
// 与 app-lynx 的 downloadsViewModel.test.ts 同源同规格（双端差分对齐）。
...
import type { DownloadStatus, DownloadTask } from "@/utils/downloadQueueCore";
...
describe("状态文案（spec §7）", () => {
  ...
  it("progressText：下载中显示百分比；失败带错误；其余显示状态", () => {
    ...
    expect(progressText(task({ id: "x", status: "failed", error: "HTTP 404" }))).toBe(
      "失败：HTTP 404",
    );
