...
describe.skipIf(!process.env.PIXIV_REFRESH_TOKEN)("agent-browser 图床代理设置", () => {
  ...
  it("图床设置页可打开", async () => {
    // 直接导航到图床设置页
    await driver.navigate("/image-host");
    await SLEEP(3000);
  ...
  it("toggle 开关 → 取消确认 → 开关保持关闭", async () => {
    ...
    // 点击开关（触发确认对话框）
    // 用 evaluate 操作 fluent-switch（避免 Shadow DOM 选择器问题）
    await driver.evaluate(
      `document.querySelector('fluent-switch[aria-label="启用图床代理"]')?.click()`,
    );
    await SLEEP(1500);
...
describe.skipIf(!process.env.PIXIV_REFRESH_TOKEN)("agent-browser 图片缓存设置", () => {
  ...
  it("导航到 /image-cache 渲染页面", async () => {
    await driver.navigate("/image-cache");
    await SLEEP(3000);
...
describe.skipIf(!process.env.PIXIV_REFRESH_TOKEN)("agent-browser 导航与路由", () => {
  ...
  it("未知路由显示 fallback 页面", async () => {
    await driver.navigate("/this-route-does-not-exist");
    await SLEEP(3000);
