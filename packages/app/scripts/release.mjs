...
// #250：minWebVersion 覆写值（--min-web=x.y.z，解析风格对齐 --variants=）。
// 缺省 = 继承旧 version.json 的 minWebVersion（step 2 覆写前读取）；旧文件无该字段 = 不设门槛。
const minWebArg = process.argv
  .slice(2)
  .find((a) => a.startsWith("--min-web="))
  ?.slice("--min-web=".length);
if (minWebArg !== undefined && !/^\d+\.\d+\.\d+$/u.test(minWebArg.trim())) {
  console.error("[release] ❌ --min-web 值无效，需要 x.y.z 格式（如 --min-web=4.21.0）");
  process.exit(1);
}
...
async function main() {
  ...
  await step(2, "更新版本号", async () => {
    ...
    if (minWebVersion === undefined) {
      try {
        const prevVerJson = JSON.parse(await readText("../../packages/website/version.json"));
        if (typeof prevVerJson.minWebVersion === "string") {
...
main().catch(async (error) => {
  ...
  if (error.stepName) {
    ...
    if (error.stepN === 1) {
      ...
      console.error(
        `     2. 上传资产（APK 与 web bundle 三件套；若 release 已存在可跳过第 1 步；仅列未成功包）:`,
      );
