 ...
 *   <li><b>断言范围声明（round-2 复审 M6）</b>：本文件只断言**资源值已登记**（{@code obtainStyledAttributes}
 *       读得到声明），**不**断言「该值在设备端生效」——plate 生效依赖父主题
 *       {@code Theme.SplashScreen.IconBackground}，而主轨 {@code AppTheme.NoActionBarLaunch} 的父
 *       仍是 {@code Theme.SplashScreen} → 主轨 plate「已登记未生效」（有意差异，不是本文件的回归对象；
 *       生效性走查见 docs/specs/lynx-night-mode-walkthrough.md T3-3c）</li>
 *   <li>为何不能进一步断言「生效」：plate 生效依赖父主题 {@code Theme.SplashScreen.IconBackground}
 *       （API 31+ 把平台 {@code android:windowSplashScreenIconBackgroundColor} 映射回本 app attr），
 *       该父链属依赖库资源语义，**JVM 无法断言**：Robolectric 不解析依赖 styles 里的 {@code ?attr}
...
public class LynxSplashThemeResourcesTest {
    ...
    public void darkSplashTheme_usesSkyDarkSurfaceAndPlate_definedInValues() {
        ...
        // 断言语义 = **资源值断言**（两支手动主题的父 = Theme.SplashScreen.IconBackground，
        // 故其 plate 经 API 31+ 平台属性映射生效；生效性证据 = walkthrough T3-3b/T3-3a 目检）
    ...
    @Test
    public void launchTheme_resourceValuesInValues_plateRegisteredButNotEffective() {
        // manifest 主轨（system 跟随）：亮色配置下资源值为白底 + 白 plate。
        // 断言范围 = **资源值已登记**：底（windowSplashScreenBackground）对系统 splash 生效；
        // plate 项的生效性**未接线**——主轨父保持 Theme.SplashScreen（非 IconBackground），
        // 故主轨 plate「已登记未生效」（有意差异；生效性见 walkthrough T3-3c）。此处不得
        // 声称「主轨暗色 splash 有圆盘」——亮色 plate==底 本就无可见差异。
