      ...
      <div class="px-4 pb-1 flex flex-col gap-1">
        ...
        <div class="flex items-center gap-2 px-1 py-1.5">
          <label class="flex items-center gap-1 text-[var(--colorNeutralForeground3)] [font-size:var(--fontSizeBase200)] whitespace-nowrap">
            从
            <input type="date" value={dateStartStr()} onInput={(e) => setDateStartStr(e.currentTarget.value)} class="history-date-input" aria-label="开始日期" />
          </label>
          <span class="text-[var(--colorNeutralForeground3)] [font-size:var(--fontSizeBase200)]">—</span>
          <label class="flex items-center gap-1 text-[var(--colorNeutralForeground3)] [font-size:var(--fontSizeBase200)] whitespace-nowrap">
            至
            <input type="date" value={dateEndStr()} onInput={(e) => setDateEndStr(e.currentTarget.value)} class="history-date-input" aria-label="结束日期" />
          </label>
      ...
      <Show
        ...
        fallback={
          <div class="flex-1 flex flex-col items-center justify-center gap-3">
            ...
            <span style={{ "font-size": "var(--fontSizeBase300)", color: "var(--colorNeutralForeground3)" }}>
              加载历史记录...
            </span>
          </div>
        }
            ...
            <div class="flex-1 flex flex-col items-center justify-center gap-3">
              <Show when={hasActiveFilters()}>
                ...
                <div class="flex flex-col items-center gap-1">
                  <span style={{ "font-size": "var(--fontSizeBase300)", color: "var(--colorNeutralForeground2)" }}>未找到匹配的记录</span>
                  <button onClick={clearFilters} class="text-[var(--colorBrandForeground1)] [font-size:var(--fontSizeBase200)] font-medium appearance-none border-none bg-transparent cursor-pointer hover:underline active:opacity-70 focus-visible:outline focus-visible:outline-[length:var(--strokeWidthThick)] focus-visible:outline-offset-[var(--strokeWidthThick)] focus-visible:outline-[color:var(--colorStrokeFocus2)]">清除筛选条件</button>
                </div>
              </Show>
              <Show when={!hasActiveFilters()}>
                <FluentIcon name="history" size={48} />
                <div class="flex flex-col items-center gap-1">
                  <span style={{ "font-size": "var(--fontSizeBase300)", color: "var(--colorNeutralForeground2)" }}>暂无浏览记录</span>
                  <span style={{ "font-size": "var(--fontSizeBase200)", color: "var(--colorNeutralForeground3)" }}>浏览过的作品会出现在这里</span>
                </div>
              </Show>
            </div>
          }
        >
          <div class="px-4 pb-1" style={{ "font-size": "var(--fontSizeBase200)", color: "var(--colorNeutralForeground3)" }}>
          ...
          <div class="flex-1 overflow-auto px-4 pb-20">
            {items().map((item) => {
              if (item.type === "header") {
                return <div class="mt-2 mb-1" style={{ "font-size": "var(--fontSizeBase200)", color: "var(--colorNeutralForeground3)", "font-weight": "var(--fontWeightSemibold)" }}>{item.dateLabel}</div>;
              }
              const e = item.entry;
              ...
              return (
                ...
                >
                  <div class="relative flex-shrink-0 w-10 h-10 rounded-[var(--borderRadiusMedium)] overflow-hidden" style={{ background: "var(--colorNeutralBackground2)" }}>
                    ...
                    {hideByR18 && <span class="absolute flex items-center justify-center rounded-[var(--borderRadiusSmall)]" style={{ top: "var(--strokeWidthThick)", left: "var(--strokeWidthThick)", "font-size": "var(--fontSizeBase100)", padding: "1px 3px", background: "var(--colorStatusDangerBackground2)", color: "var(--colorStatusDangerForeground1)", "font-weight": "var(--fontWeightBold)" }}>{e.xRestrict === 2 ? "R18G" : "R-18"}</span>}
                  </div>
                  ...
                  <button class="history-delete-btn" onClick={(ev) => { ev.stopPropagation(); removeHistoryEntry(e.key); }} aria-label={`删除 ${e.title}`}>✕</button>
                </div>
              );
            })}
          </div>
