 ...
 *
 * full 到位前主 img 不挂载（displaySrc 为空串），thumb 层常驻不卸载（防白帧/兜底）；
 * 切换完成后缩略层也不卸载——位于主层下方零视觉成本，主图后续 onError 时兜底。
 */
...
export function createProgressiveImage(options: ProgressiveImageOptions): ProgressiveImageState {
  const [thumbSrc, setThumbSrc] = createSignal<string | undefined>(undefined);
  ...
  createEffect(() => {
    ...
    // 渐进：thumb 先行占位；主 img 不挂载（防白帧），thumb 层常驻兜底
  ...
  return { thumbSrc, displaySrc, onThumbError, onDisplayError, failed };
}
