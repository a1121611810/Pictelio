...
// 纯函数 + 注入 saveOne（IO 边界在 native/GallerySaver.ts），node 可测（测试硬约束 #1）。
...
export function buildSaveFileName(illustId: number, page: number | undefined, url: string): string {
  ...
  return page === undefined
    ? `Pictelio_${illustId}.${ext}`
    : `Pictelio_${illustId}_p${page}.${ext}`;
}

/**
 * 原图 URL 列表（与 IllustDetail 原 originalImageUrls() 同语义）：
 * 多页 = meta_pages[].image_urls.original ?? large；单页 = meta_single_page.original_image_url ?? large。
 * 保存恒为原图档（spec §2 A4），缺失沿既有回退链降级。
 */
 ...
 * 失败明细 console.warn（无静默降级）并由调用方以 toast/状态文本汇报聚合结果。
 */
