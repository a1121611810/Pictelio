...
/**
 * 静音词表「组装时快照」读（spec 边界 #4：已渲染列表不回溯移除，v1 只影响后续组装）。
 * untrack 使词表 signal 变化不进入调用方的 tracked 依赖——FeedList `<For>` 内的既有
 * memo/视图不会因长按静音而立即重滤移卡；词表仅在下次组装（查询 transform / 翻页 /
 * 刷新重新调用过滤函数）时以新快照生效。管理页（MuteTagSheet）与轻提示直接消费
 * muteTagStore 响应式面，不经本函数，不受影响。
 */
function mutedSnapshot(): ReadonlySet<string> {
  return untrack(mutedTags);
}
 ...
 * 静音词表同模式（ADR-0187 D3）且为 untrack 快照（spec 边界 #4），集合为空时后续
 * 逐条判定短路。
 */
export function filterFeedIllusts(illusts: PixivIllust[]): PixivIllust[] {
  const blocked = blockedIds();
  const muted = mutedSnapshot();
